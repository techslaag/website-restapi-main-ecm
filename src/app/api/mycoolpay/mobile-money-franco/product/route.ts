...
export async function POST(request: Request) {
  return authMiddleware(request, async (user) => {
    try {
      ...
      console.log("[MyCoolPay Product] Processing request:", {
        userId: user.id,
        productId: bodyPayload.productId,
        phoneNumber: bodyPayload.phoneNumber.substring(0, 8) + "***",
        userIp: bodyPayload.userIp
      });
      ...
      return await countryAndExchangeRatesMiddleware(
        ...
        async (ipData, exchangeRates) => {
          try {
            console.log("[MyCoolPay Product] Country data:", {
              country: ipData.countryAlpha2Code,
              currency: ipData.currencyCode,
              isSupported: ["xaf", "xof"].includes(ipData.currencyCode.toLowerCase())
            });
            ...
            console.log("[MyCoolPay Product] Product found:", {
              id: product.id,
              name: product.name,
            ...
            console.log("[MyCoolPay Product] Payment amount calculated:", {
