import IPaginateResponse from "@/interfaces/IPaginateResponse";
import IPodcast, { toIPodcast, toUpdateIPodcast } from "@/interfaces/IPodcast";
import IPostMedia from "@/interfaces/IPostMedia";
import { getPaginationMetaData } from "@/lib/utils/databaseUtils";
import prisma from "@/lib/prisma";
import { extractQueryParams, toSafeJSON } from "@/lib/utils/index";
import { Prisma } from "@prisma/client";

export const dynamic = "force-dynamic";

export async function GET(req: Request) {
  const queryParams = extractQueryParams(req);

  const page = Number(queryParams.page ?? 1),
    limit = Number(queryParams.limit ?? 25);

  // filter to be used for both pagination meta and the data list
  const whereQuery: Prisma.mod180_postsWhereInput = {
    post_type: "podcast",
    post_status: "publish",
  };

  const paginationMeta = await getPaginationMetaData(
    "mod180_posts",
    page,
    limit,
    whereQuery,
  );

  const podcasts = await prisma.mod180_posts.findMany({
    where: whereQuery,
    ...paginationMeta.query,
    orderBy: {
      post_date_gmt: "desc",
    },
    select: {
      ID: true,
      post_name: true,
      post_excerpt: true,
      post_title: true,
      post_date: true,
      post_date_gmt: true,
      termRelationships: {
        include: {
          taxonomy: {
            include: {
              term: true,
            },
          },
        },
      },
      meta: {
        select: {
          meta_key: true,
          meta_value: true,
        },
      },
      children: {
        select: {
          ID: true,
          guid: true,
          post_type: true,
          post_excerpt: true,
          post_mime_type: true,
          post_title: true,
          post_date: true,
          termRelationships: {
            include: {
              taxonomy: {
                include: {
                  term: true,
                },
              },
            },
          },
          meta: {
            select: {
              meta_key: true,
              meta_value: true,
            },
          },
        },
      },
    },
  });

  if (podcasts == null) {
    return Response.json([]);
  }

  const formattedResponse = toSafeJSON(podcasts);
  const parsedPodcasts = await Promise.all(
    formattedResponse.map(async (podcast: any) => {
      const parsedPodcast = toIPodcast(podcast);

      const media = await prisma.mod180_posts.findUnique({
        where: {
          ID: Number(parsedPodcast.mediaId),
        },
        select: {
          ID: true,
          guid: true,
          post_type: true,
          post_excerpt: true,
          post_mime_type: true,
          post_title: true,
          post_date: true,
          termRelationships: {
            include: {
              taxonomy: {
                include: {
                  term: true,
                },
              },
            },
          },
          meta: {
            select: {
              meta_key: true,
              meta_value: true,
            },
          },
        },
      });

      if (media == null) {
        return parsedPodcast;
      } else {
        const parsedMedia = {
          id: media.ID.toString(),
          mediaType: media.post_mime_type?.split("/")[0] ?? "file",
          mimeType: media.post_mime_type,
          title: media.post_title,
          altText: media.meta?.find(
            (item) => item.meta_key === "_wp_attachment_image_alt",
          )?.meta_value,
          caption: media.post_excerpt,
          sourceUrl: media.guid,
          mediaPath: media.meta?.find(
            (item) => item.meta_key === "_wp_attached_file",
          )?.meta_value!,
          date: media.post_date,
        } as IPostMedia;

        return toUpdateIPodcast(parsedPodcast, parsedMedia);
      }
    }),
  );

  return Response.json(
    toSafeJSON<IPaginateResponse<IPodcast>>({
      ...paginationMeta.meta,
      items: parsedPodcasts,
    }),
  );
}
